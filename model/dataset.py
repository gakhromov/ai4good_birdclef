...
class BirdClefMixUpMelDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        # load label (avoid onehot like fastai does it)
        ...
        # load label
        label = torch.tensor(self.mapping.index(self.pri_dec[index_mix])).type(torch.LongTensor)
        labels.append(label)
...
def onehot_primary(label, n_classes=152):
    x = torch.zeros(152)
    x[label] = 1
    return x
